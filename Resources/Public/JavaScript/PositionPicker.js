...
define([
  ...
  'TYPO3/CMS/PoiMap/GoogleMapsLoader'
], function($, FormEngine, GoogleMapsLoader) {
  'use strict';
   ...
   */
  var PositionPicker = function(container) {
    this.$container = $(container);
    this.$view = $(this.$container.data('view'));
    this.$outputActual = FormEngine.getFieldElement(
      this.$container.data('output-name')
    );
    this.$outputVisual = this.$container
      .closest('form')
      .find(
  ...
  PositionPicker.prototype.setMarkerPosition = function(position) {
